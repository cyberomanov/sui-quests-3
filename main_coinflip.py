...
def main_play_game(sui_config: SuiConfig, associated_kiosk_addr: str, bullshark_addr: str, bullshark: bool = True,
    ...
    try:
        ...
        if balance.float > bet_amount:
            ...
            if quest_pass:
                result = play_coinflip_dummy_tx(sui_config=sui_config,
...
def single_executor(sui_config: SuiConfig):
    try:
        ...
        if 'QuestPass' in associated_kiosk_addr:
            quest_pass = True
        else:
        ...
        if bullshark_addr or capy_addr or quest_pass:
            ...
            if not broken:
                if capy_addr:
                    ...
                    logger.success(f'{str(sui_config.active_address)} | '
                                   f'has coinflipped {flips_per_session} times with QUEST_PASS.')
                return
            else:
                return
        else:
