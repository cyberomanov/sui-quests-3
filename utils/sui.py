...
from datatypes.sui import Arrow, Sui8192MoveResult, SuiTxResult, CoinflipSide, SuiBalance, SuiTransferConfig, SuiTx
from utils.other import short_address
...
def play_coinflip_tx(sui_config: SuiConfig,
                     ...
                     bet_amount: int,
                     bullshark: bool = True) -> SuiTxResult:
    transaction = init_transaction(sui_config=sui_config, merge_gas_budget=True)
    ...
    transaction.move_call(
        ...
        arguments=[
            ...
            split,
            ObjectID(GAME_COINFLIP_ARG5 if bullshark else GAME_COINFLIP_ARG5_ALT),
            ObjectID(associated_kiosk_addr),
            SuiAddress(bullshark_addr)
        ],
        type_arguments=[
            '0x2::sui::SUI',
            GAME_COINFLIP_BULLSHARK_TYPE if bullshark else GAME_COINFLIP_CAPY_TYPE
        ]
    )
    return build_and_execute_tx(sui_config=sui_config, transaction=transaction)


def play_suilette_tx(sui_config: SuiConfig,
                     color_index: int,
                     game_object_id: str,
    ...
    transaction.move_call(
        target=SuiString(SUILETTE_MAIN_TARGET),
        arguments=[
            split,
            SuiU8(color_index),
            [],
            SuiAddress(game_object_id),
            ObjectID(SUILETTE_ARG5),
            [SuiString(str(sui_config.active_address))],
            [],
            []
        ],
        type_arguments=[
